...
import { findLocalAppBySlug } from "../core/config/localAppStore.js";
const router = Router();
...
router.get("/:slug/routes", async (req, res) => {
    ...
    try {
        ...
        const { config } = parseConfigFromObject(app.config);
...
router.use("/:slug", optionalAuth, async (req, res, next) => {
    const slug = Array.isArray(req.params.slug)
        ? req.params.slug[0]
        : req.params.slug;
    if (!slug) {
    ...
    try {
        ...
        const entityRouter = buildRuntimeRouter(config, slug);
