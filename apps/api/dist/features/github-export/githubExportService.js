...
export function startGitHubExport(userId, request) {
...
export function getGitHubExportStatus(jobId) {
    return exportJobs.get(jobId) ?? null;
}
async function runGitHubExport(userId, request, jobId) {
    ...
    try {
        ...
        if (!parsed.valid) {
            throw new AppError(`App \"${request.appSlug}\" has an invalid configuration and cannot be exported`, 422, "EXPORT_GENERATION_FAILED");
        }
...
async function createGitHubRepository(token, repositoryName, isPrivate) {
...
function mapGitHubError(status, payload) {
...
async function initializeRepository(directory, cloneUrl, repositoryName) {
...
async function pushRepository(directory) {
...
function buildProjectFiles(config, repositoryName) {
    return [
...
function buildPackageJson(repositoryName) {
...
function buildFrontendApp(config) {
...
const entityFields = ${JSON.stringify(config.entities.reduce((acc, entity) => {
...
function buildPageSummary(name, slug, componentCount) {
...
function buildBackendApp(config) {
...
function buildDatabaseSchema(config) {
...
function buildWorkflowFile(workflows) {
...
function buildReadme(config, repositoryName) {
