...
async function executeAction(workflow, action, triggerData, userId) {
    switch (action.type) {
        ...
        case "db_write": {
            ...
            const valueList = columns.map((column) => serializeSqlValue(data[column]));
            const query = `INSERT INTO "${table}" (${columnList.join(", ")}) VALUES (${valueList.join(", ")})`;
            await prisma.$executeRawUnsafe(query);
            return;
        }
        case "send_email": {
            const recipient = typeof action.config.to === "string"
                ? action.config.to
                : typeof action.config.email === "string"
                    ? action.config.email
                    : undefined;
            const subject = typeof action.config.subject === "string" ? action.config.subject : undefined;
            console.log(`[Workflow] Email action - would send to: ${recipient ?? "unknown"} subject: ${subject ?? "(none)"}`);
...
export async function executeWorkflow(workflow, triggerData, userId) {
    ...
    for (const action of workflow.actions) {
        try {
            ...
            console.error(`[Workflow] Action failed for workflow "${workflow.name}" action type="${action.type}":`, error);
        }
    }
}
export async function executeWorkflowsForEvent(workflows, trigger, entity, data, userId) {
    const matchingWorkflows = workflows.filter((workflow) => workflow.trigger === trigger && workflow.entity === entity);
