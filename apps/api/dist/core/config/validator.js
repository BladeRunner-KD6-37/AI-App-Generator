            ...
            ? entity.fields.filter(isObject).map((field) => ({
                ...
                relation: isObject(field.relation)
                    ? {
                        ...
                        type: field.relation.type === "many-to-one" || field.relation.type === "many-to-many"
                            ? field.relation.type
                            : "one-to-many",
                    }
                    : undefined,
        ...
        const relations = Array.isArray(entity.relations)
            ? entity.relations.filter(isObject).map((relation) => ({
                ...
                relation: {
                    ...
                    type: relation.type === "many-to-one" || relation.type === "many-to-many"
                        ? relation.type
                        : "one-to-many",
                },
            }))
    ...
    return raw
        ...
        if (Array.isArray(page.components)) {
            return {
                ...
                slug: typeof page.slug === "string" && page.slug.length > 0
                    ? page.slug
                ...
                components: page.components.filter(isObject).map((component) => ({
                    ...
                    fields: Array.isArray(component.fields)
                        ? component.fields.filter((field) => typeof field === "string")
                        : undefined,
                })),
                roles: Array.isArray(page.roles)
                    ? page.roles.filter((role) => typeof role === "string")
                    : undefined,
            };
        }
        ...
        return {
            name: title,
            slug: typeof page.slug === "string" && page.slug.length > 0 ? page.slug : slugify(title),
    ...
    result.data.pages.forEach((page) => {
        page.components.forEach((comp) => {
            ...
            if (!knownTypes.includes(comp.type)) {
                warnings.push(`Page "${page.name}" has unknown component type "${comp.type}" — will render fallback`);
            }
        });
    });
    return {
